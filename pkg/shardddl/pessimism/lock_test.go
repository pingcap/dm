...
func (t *testLock) TestLock(c *C) {
	...
	// DDLs mismatch.
	synced, remain, err := l1.TrySync(etcdTestCli, source1, DDLs[1:], []string{source1}, nil)
	...
	// synced.
	synced, remain, err = l1.TrySync(etcdTestCli, source1, DDLs, []string{source1}, nil)
	c.Assert(err, IsNil)
	...
	// join a new source.
	synced, remain, err = l2.TrySync(etcdTestCli, source1, DDLs, []string{source2, source3}, nil)
	c.Assert(err, IsNil)
	c.Assert(synced, IsFalse)
	...
	// sync other sources.
	synced, remain, err = l2.TrySync(etcdTestCli, source2, DDLs, []string{}, nil)
	c.Assert(err, IsNil)
	c.Assert(synced, IsFalse)
	c.Assert(remain, Equals, 1)
	c.Assert(l2.Ready(), DeepEquals, map[string]bool{
	...
	synced, remain, err = l2.TrySync(etcdTestCli, source3, DDLs, nil, nil)
	c.Assert(err, IsNil)
	c.Assert(synced, IsTrue)
	c.Assert(remain, Equals, 0)
	c.Assert(l2.Ready(), DeepEquals, map[string]bool{
