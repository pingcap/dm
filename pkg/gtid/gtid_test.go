...
func (s *testGTIDSuite) TestMySQLGTIDTruncate(c *C) {
	var (
		g1, _       = ParserGTID("mysql", "00c04543-f584-11e9-a765-0242ac120002:100")
		g2, _       = ParserGTID("mysql", "00c04543-f584-11e9-a765-0242ac120002:100")
		gNil        *mySQLGTIDSet
		gEmpty, _   = ParserGTID("mysql", "")
	...
	for _, cs := range cases {
		bg, err := ParserGTID("mysql", cs.before)
		c.Assert(err, IsNil)
		eg, err := ParserGTID("mysql", cs.end)
		c.Assert(err, IsNil)
		ag, err := ParserGTID("mysql", cs.after)
		c.Assert(err, IsNil)
		err = bg.Truncate(eg)
		if cs.hasError {
