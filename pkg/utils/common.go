...
func FetchTableTransferColumnRule(r *router.Table, schema, table, source string) ([]string, []string) {
	var cols []string
	var vals []string
	...
	if rule.Extractors.TableExtractor != nil {
		cols = append(cols, rule.Extractors.TableExtractor.TargetColumn)
		vals = append(vals, rule.Extractors.MatchVal(table, rule.Extractors.TableExtractor))
	}

	if rule.Extractors.SchemaExtractor != nil {
		cols = append(cols, rule.Extractors.SchemaExtractor.TargetColumn)
		vals = append(vals, rule.Extractors.MatchVal(schema, rule.Extractors.SchemaExtractor))
	}

	if rule.Extractors.SourceExtractor != nil {
		cols = append(cols, rule.Extractors.SourceExtractor.TargetColumn)
		vals = append(vals, rule.Extractors.MatchVal(source, rule.Extractors.SourceExtractor))
	}
	return cols, vals
}