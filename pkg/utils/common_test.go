...
func (s *testCommonSuite) TestFetchTableTransferColumnRule(c *C) {
	...
	transferCols, transferVals := FetchTableTransferColumnRule(r, "schema_s1", "table_t1", "source_s1")
	c.Assert(expected[0], DeepEquals, transferCols)
	c.Assert(expected[1], DeepEquals, transferVals)

	// only schema rules
	transferCols2, transferVals2 := FetchTableTransferColumnRule(r, "schema_s2", "a_table_t2", "source_s2")
	c.Assert(expected[2], DeepEquals, transferCols2)
	c.Assert(expected[3], DeepEquals, transferVals2)
}
