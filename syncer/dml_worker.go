...
func dmlWorkerWrap(inCh chan *job, syncer *Syncer) chan *job {
	dmlWorker := &DMLWorker{
		...
		addFlush:     syncer.addFlushCheckpointSnapshotJobFunc,
		inCh:         inCh,
...
func (w *DMLWorker) run() {
	...
	for j := range w.inCh {
		...
		case flushCheckpointSnapshot:
			// TODO: avoid generate too many flushCheckpointSnapshot
			w.wg.Add(w.workerCount)
			// flush for every DML queue
			for i, jobCh := range jobChs {
				startTime := time.Now()
...
func (w *DMLWorker) executeJobs(queueID int, jobCh chan *job) {
	...
	for j := range jobCh {
		...
		if j.tp != flush && j.tp != conflict && j.tp != flushCheckpointSnapshot {
			if len(jobs) == 0 {
		...
		if j.tp == flushCheckpointSnapshot {
			w.addFlush(j.checkPointSnapshotID, queueID)
		}
		...
		// TODO: catch potential panic caused by w.wg.Done() here?
		if j.tp == conflict || j.tp == flush || j.tp == flushCheckpointSnapshot {
			w.wg.Done()
		}
