...
func (s *Syncer) Status() interface{} {
	...
	if err != nil {
		s.logger.Warn("fail to get master status", zap.Error(err))
	}

	syncerPos := s.checkpoint.FlushedGlobalPoint()
	if err != nil {
		s.logger.Warn("fail to get flushed global point", zap.Error(err))
	}
	st := &pb.SyncStatus{
		TotalEvents:  total,
		TotalTps:     totalTps,
	...
	if err != nil {
		s.logger.Debug("fail to parse real mysql position", zap.Stringer("position", syncerPos), log.ShortError(err))
	}
