...
func (s *testCheckpointSuite) testTableCheckPoint(c *C, cp CheckPoint) {
	...
	c.Assert(err, IsNil)
	ti, err := s.tracker.GetTable(schemaName, tableName)
	c.Assert(err, IsNil)
	...
	_, err = s.tracker.GetTable(schemaName, tableName)
	...
	c.Assert(err, IsNil)
	ti2, err := s.tracker.GetTable(schemaName, tableName)
	c.Assert(err, IsNil)
	cp.SaveTablePoint(table, binlog.Location{Position: pos2}, ti2)
	cp.Rollback(s.tracker)
	ti11, err := s.tracker.GetTable(schemaName, tableName)
	c.Assert(err, IsNil)
