...
func (s *testSyncerSuite) TestSharding(c *C) {
	...
	for i, _case := range testCases {
		...
		syncer.sgk.dbConn = &DBConn{cfg: s.cfg, baseConn: conn.NewBaseConn(shardGroupDBConn, &retry.FiniteRetryStrategy{}, nil)}
		syncer.sgk.prepare()
		syncer.initShardingGroups()

		// mock syncer.checkpoint.Init() function
		syncer.checkpoint.(*RemoteCheckPoint).dbConn = &DBConn{cfg: s.cfg, baseConn: conn.NewBaseConn(checkPointDBConn, &retry.FiniteRetryStrategy{}, nil)}
