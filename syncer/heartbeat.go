...
type Heartbeat struct {
	...
	primary       *sql.DB
	secondariesTs map[string]float64 // task-name => secondary (syncer) ts
...
func GetHeartbeat(cfg *HeartbeatConfig) (*Heartbeat, error) {
	once.Do(func() {
		heartbeat = &Heartbeat{
			...
			secondariesTs: make(map[string]float64),
...
func (h *Heartbeat) AddTask(name string) error {
	...
	if _, ok := h.secondariesTs[name]; ok {
	...
	h.secondariesTs[name] = 0 // init to 0
	return nil
}
...
func (h *Heartbeat) RemoveTask(name string) error {
	...
	delete(h.secondariesTs, name)

	if len(h.secondariesTs) == 0 {
		// cancel work
		h.cancel()
		h.cancel = nil
		h.wg.Wait()
...
func (h *Heartbeat) TryUpdateTaskTs(taskName, schema, table string, data [][]interface{}) {
	...
	select {
	case h.lock <- struct{}{}:
		if _, ok := h.secondariesTs[taskName]; ok {
			h.secondariesTs[taskName] = h.timeToSeconds(t)
		}
		<-h.lock
	default:
...
func (h *Heartbeat) calculateLag(ctx context.Context) error {
	...
	select {
	case h.lock <- struct{}{}:
		for taskName, ts := range h.secondariesTs {
			if ts == 0 {
