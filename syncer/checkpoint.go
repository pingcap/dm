...
type binlogPoint struct {
	...
	// flushedLocation.location which flushed permanently
...
func (b *binlogPoint) flushBy(sp tablePoint) {
	b.Lock()
	defer b.Unlock()
	b.flushedLocation.location = sp.location
	b.flushedLocation.ti = sp.ti
}
...
func (b *binlogPoint) outOfDate() bool {
	...
	return binlog.CompareLocation(b.location.location, b.flushedLocation.location, b.enableGTID) > 0
}
...
type CheckPoint interface {
	...
	// FlushGlobalPointsExcept flushes the global checkpoint and tables'
	// checkpoints except exceptTables with specified snapshot.
	FlushSnapshotPointsExcept(tctx *tcontext.Context, snapshot int, exceptTables []*filter.Table, extraSQLs []string, extraArgs [][]interface{}) error
...
func (cp *RemoteCheckPoint) FlushPointsExcept(tctx *tcontext.Context, exceptTables []*filter.Table, extraSQLs []string, extraArgs [][]interface{}) error {
	...
	if cp.globalPoint.outOfDate() || cp.globalPointCheckOrSaveTime.IsZero() || cp.needFlushSafeModeExitPoint {
	...
	for schema, mSchema := range cp.points {
		for table, point := range mSchema {
			...
			if point.outOfDate() {
				tiBytes, err := json.Marshal(point.location.ti)
				if err != nil {
...
// FlushSnapshotPointsExcept implements CheckPoint.FlushSnapshotPointsExcept.
func (cp *RemoteCheckPoint) FlushSnapshotPointsExcept(
...
func (cp *RemoteCheckPoint) CheckGlobalPoint() bool {
	cp.RLock()
	defer cp.RUnlock()
	t := cp.globalPointCheckOrSaveTime
	cp.globalPointCheckOrSaveTime = time.Now()
	return time.Since(t) >= time.Duration(cp.cfg.CheckpointFlushInterval)*time.Second
}
