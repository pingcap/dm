...
type OnlinePlugin interface {
	...
	// returns sqls, error
	Apply(tctx *tcontext.Context, tables []*schemapkg.Table, statement string, stmt ast.StmtNode) ([]string, error)
...
// returns ddls, error.
func (r *RealOnlinePlugin) Apply(tctx *tcontext.Context, tables []*schemapkg.Table, statement string, stmt ast.StmtNode) ([]string, error) {
	if len(tables) < 1 {
