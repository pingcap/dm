...
func (s *testFilterSuite) TestFilterQueryEvent(c *C) {
	syncer := &Syncer{}
	// build in skipDDL
	skipped, err := syncer.filterQueryEvent(nil, nil, "DROP TEMPORARY TABLE IF EXISTS `foo`.`bar` /* generated by server */")
	c.Assert(err, IsNil)
	...
	for _, ca := range cases {
		...
		c.Assert(err, IsNil)
		skipped, err = syncer.filterQueryEvent(ca.tables, stmt, ca.sql)
		c.Assert(err, IsNil)
