...
func (sg *ShardingGroup) Tables() (tables []*schemapkg.Table) {
	sources := sg.Sources()
...
func (k *ShardingGroupKeeper) AddGroup(targetTable *schemapkg.Table, sourceIDs []string, meta *shardmeta.ShardingMeta, merge bool) (needShardingHandle bool, group *ShardingGroup, synced bool, remain int, err error) {
	...
	schemaID, _ := utils.GenTableID(targetTable.Schema, "")
	targetTableID, _ := utils.GenTableID(targetTable.Schema, targetTable.Name)
...
func (k *ShardingGroupKeeper) TrySync(
	originTable, targetTable *schemapkg.Table, location, endLocation binlog.Location, ddls []string) (
	needShardingHandle bool, group *ShardingGroup, synced, active bool, remain int, err error) {
	targetTableID, schemaOnly := utils.GenTableID(targetTable.Schema, targetTable.Name)
	if schemaOnly {
	...
	synced, active, remain, err = group.TrySync(originTable.String(), location, endLocation, ddls)
...
func (k *ShardingGroupKeeper) InSyncing(originTable, targetTable *schemapkg.Table, location binlog.Location) bool {
	group := k.Group(targetTable)
	if group == nil {
		return false
	}
	return !group.CheckSyncing(originTable.String(), location)
}
...
func (k *ShardingGroupKeeper) UnresolvedTables() (ids map[string]bool, tables []*schemapkg.Table) {
	k.RLock()
	defer k.RUnlock()
