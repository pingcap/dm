...
1. The following executables must be copied or generated or linked into these locations, `mydumper` and `sync_diff_inspector` can be downloaded from [tidb-enterprise-tools-latest-linux-amd64](http://download.pingcap.org/tidb-enterprise-tools-latest-linux-amd64.tar.gz):
    ...
    * `gh-ost`                  # must add to path, or you can `export GHOST_BINARY=/path/to/gh-ost-binary`
    * `pt-online-schema-change` # must add to path, or you can `export PTOSC_BINARY=/path/to/pt-osc-binary`
...
2. Run `make integration_test` to execute the integration tests. This command will
    ...
    > The online ddl test using pt-osc doesn't work if the upstream MySQL has different connect port from bind port (often caused by port forwarding via NAT). In this case you must specific the real ip and port of MySQL. Othersise you can skip online ddl test by `export ONLINE_DDL_ENABLE=false`.
