...
function run() {
	export GO_FAILPOINTS="github.com/pingcap/dm/pkg/streamer/SetHeartbeatInterval=return(1);github.com/pingcap/dm/syncer/waitingJob=return(true)"
	...
	kill $pid
	check_log_contain_with_retry 'waiting job' $WORK_DIR/worker1/log/dm-worker.log
	check_log_contain_with_retry 'waiting job' $WORK_DIR/worker2/log/dm-worker.log
	check_sync_diff $WORK_DIR $cur/conf/diff_config.toml
