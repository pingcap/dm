...
coverage: retool_setup
	retool do gocovmerge "$(TEST_DIR)"/cov.* | grep -vE ".*.pb.go|.*.__failpoint_binding__.go" > "$(TEST_DIR)/all_cov.out"
	retool do gocovmerge "$(TEST_DIR)"/cov.unit_test*.out | grep -vE ".*.pb.go|.*.__failpoint_binding__.go|.*dm/ctl.*|.*dm/portal.*|.*dm/tracer.*|.*loader.*|.*mydumper.*" > $(TEST_DIR)/unit_test.out
ifeq ("$(JenkinsCI)", "1")
