...
coverage: coverage_fix_cover_mode retool_setup
	retool do gocovmerge "$(TEST_DIR)"/cov.* | grep -vE ".*.pb.go|.*.pb.gw.go|.*.__failpoint_binding__.go|.*debug-tools.*|.*portal.*|.*chaos.*|.*ctl.*|.*loader.*|.*relay.*" > "$(TEST_DIR)/all_cov.out"
	retool do gocovmerge "$(TEST_DIR)"/cov.unit_test*.out | grep -vE ".*.pb.go|.*.pb.gw.go|.*.__failpoint_binding__.go|.*debug-tools.*|.*portal.*|.*chaos.*|.*ctl.*|.*loader.*|.*relay.*" > $(TEST_DIR)/unit_test.out
ifeq ("$(JenkinsCI)", "1")
