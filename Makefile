...
vet:
	GO111MODULE=off go get golang.org/x/tools/go/analysis/passes/shadow/cmd/shadow
	@echo "vet"
	@ $(GO) vet -composites=false $(PACKAGES) 2>&1 | awk '{print} END{if(NR>0) {exit 1}}'
	@ $(GO) vet -vettool=$(shell which shadow) $(PACKAGES) 2>&1 | tee /dev/stderr | awk '/shadows declaration|^#/{next}{count+=1} END{if(count>0) {exit 1}}'
