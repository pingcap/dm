...
func (t *testConfig) TestSubTaskAdjustFail(c *C) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			"[[0-9]+:.*:.*:.*] task name should not be empty",
		},
		{
			func() *SubTaskConfig {
				cfg := newSubTaskConfig()
		...
		},
		{
			...
			"[[0-9]+:.*:.*:.*] too long source-id not valid",
		},
		{
			func() *SubTaskConfig {
				cfg := newSubTaskConfig()
			...
			"[[0-9]+:.*:.*:.*] online scheme rtc not supported",
		},
		{
			func() *SubTaskConfig {
				cfg := newSubTaskConfig()
			...
			"[[0-9]+:.*:.*:.*] invalid timezone string: my-house:.*",
		},
	}
