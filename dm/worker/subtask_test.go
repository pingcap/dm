...
func (m *MockUnit) Process(ctx context.Context, pr chan pb.ProcessResult) {
	...
	case err := <-m.processErrorCh:
		if err == nil {
			...
			pr <- pb.ProcessResult{
				Errors: append([]*pb.ProcessError{}, unit.NewProcessError(pb.ErrorType_UnknownError, errors.ErrorStack(err))),
			}
		}
	}
}
...
func (t *testSubTask) TestSubTaskNormalUsage(c *C) {
	...
	// inject error into isFressh method
	mockLoader.InjectFreshError(true, errors.New("loader fresh error"))
	st.Run()
	c.Assert(st.CurrUnit(), Equals, mockLoader)
	c.Assert(st.Result(), NotNil)
	c.Assert(st.Result().Errors, HasLen, 1)
	c.Assert(strings.Contains(st.Result().Errors[0].Msg, "loader fresh error"), IsTrue)
	c.Assert(st.Stage(), Equals, pb.Stage_Paused)

	// remove injected error
	mockLoader.InjectFreshError(false, nil)
	st.Run()
	c.Assert(st.CurrUnit(), Equals, mockLoader)
	c.Assert(st.Result(), IsNil)
	c.Assert(st.Stage(), Equals, pb.Stage_Running)
