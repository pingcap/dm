...
const (
	...
	// defaultNamePrefix is the default prefix for this dm-worker's name
	defaultNamePrefix = "dm-worker"
)
...
func NewConfig() *Config {
	...
	fs.StringVar(&cfg.Name, "name", "", "human-readable name for this DM-worker member")
	fs.StringVar(&cfg.WorkerAddr, "worker-addr", "", "worker API server and status addr")
	fs.StringVar(&cfg.MasterAddrs, "master-addrs", "", "DM-masters' address, for example: http://master1:8261,http://master2:8261,http://master3:8261")
...
type Config struct {
	...
	// DM-masters' address, for example: http://master1:8261,http://master2:8261,http://master3:8261
	MasterAddrs string `toml:"master-addrs" json:"master-addrs"`
...
func (c *Config) verify() error {
	...
	if len(c.MasterAddrs) == 0 {
		// TODO: use terror
		//log.Error("need set master addrs")
	}
...
func (c *Config) adjust() error {
	...
	// adjust dm-worker's name, if name is empty, generate it by hostname and port,
	// for example: "dm-worker-localhost-8262"
	if c.Name == "" {
		// WorkerAddr's format may be "host:port" or ":port"
		_, port, err := net.SplitHostPort(c.WorkerAddr)
		if err != nil {
		...
		if err != nil {
			return terror.ErrWorkerGetHostnameFail.Delegate(err)
		}
