...
func (s *testTaskCheckerSuite) TestIsResumableError(c *check.C) {
	testCases := []struct {
		...
		{pb.ErrorType_ExecSQL, terror.ErrDBExecuteFailed.Delegate(&tmysql.SQLError{1062, "Duplicate entry '5' for key 'PRIMARY'", tmysql.DefaultMySQLState}, "insert into t values (5)"), false},
		{pb.ErrorType_ExecSQL, terror.ErrDBExecuteFailed.Delegate(&tmysql.SQLError{1406, "Data too long for column 'c2' at row 1", tmysql.DefaultMySQLState}, "INSERT INTO `db`.`tbl` (`c1`,`c2`) VALUES (?,?)"), false},
