...
func (t *testMaster) TestGetLeader(c *check.C) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	// create a new cluster
	cfg1 := NewConfig()
	...
	{
		res, err := s1.GetLeader(ctx, &pb.GetLeaderRequest{})
		c.Assert(err, check.IsNil)
		c.Assert(res.Name, check.Equals, cfg1.Name)
		c.Assert(res.Addr, check.Equals, cfg1.AdvertiseAddr)
	}
	...
	{
		res, err := s2.GetLeader(ctx, &pb.GetLeaderRequest{})
		c.Assert(err, check.IsNil)
		c.Assert(res.Name, check.Equals, cfg1.Name)
		c.Assert(res.Addr, check.Equals, cfg1.AdvertiseAddr)
	}
	...
	s1.Close()
	{
		res, err := s2.GetLeader(ctx, &pb.GetLeaderRequest{})
		c.Assert(err, check.IsNil)
		c.Assert(res.Name, check.Equals, "")
		c.Assert(res.Addr, check.Equals, "")
	}

	/*
		It seems that if we kill leader in 2 masters cluster,
		the second one wouldn't be the leader so follow cases would not be passed.
		But I fail to create 3 masters cluster...

		// wait the second one become the leader
		c.Assert(utils.WaitSomething(30, 100*time.Millisecond, func() bool {
...

		{
			res, err := s2.GetLeader(ctx, &pb.GetLeaderRequest{})
			c.Assert(err, check.IsNil)
			c.Assert(res.Name, check.Equals, cfg2.Name)
			c.Assert(res.Addr, check.Equals, cfg2.AdvertiseAddr)
		}
